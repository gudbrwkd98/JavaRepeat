...
public class CarHandling {
	public static void main(String[] args) {
		...
		morning.startEnginee();//객체명.호출할 메서드명() or (~)
		morning.speedUp(150); //speedUp메서드를 호출하면서 150 값 전달 
		morning.speedDown(100); // 150 - 100 = 100
		//객체명 (저장된 위치).멤버변수
		System.out.println("차의 이름은 => "  + morning.name);
		System.out.println("차의 배기량 = > " +  morning.output);
		//자주 사용되고 반복적인 코드 => 메서드를 만들어서 호출
		//-> 코드의 재사용성, 코딩양이 줄어든다.(개발시간이 단축) => 프로드램 안정화
		sonata.name = "소나타";
